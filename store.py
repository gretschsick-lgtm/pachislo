...
def get_hot_events(today, lookback_days=90):
    ...
    # 過去データから「アツいホール」Top3を抽出
    # 条件: イベント開催回数が多い＋直近でも活発
    hot_halls = conn.execute("""
        ...
        LIMIT 3
    """, {"today":today,"lb":f"-{lookback_days} days"}).fetchall()
    ...
    weekday_hot = conn.execute("""
        ...
        ORDER BY cnt DESC
        LIMIT 5
    """, {"today":today,"lb":f"-{lookback_days} days","wd":(weekday+1)%7}).fetchall()
